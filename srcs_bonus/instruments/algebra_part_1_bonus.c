...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   algebra_part_1_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: enena <enena@student.21-school.ru>         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/04 13:47:07 by enena             #+#    #+#             */
/*   Updated: 2022/01/08 18:39:50 by enena            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	get_table(float ***table)
{
	...
	*table = malloc(sizeof(float *) * 2);
	if (!(*table))
		return (error_handler(err_alloc_fail));
	(*table)[clockwise] = malloc(sizeof(float) * 4);
	if (!((*table)[clockwise]))
		return (error_handler(err_alloc_fail));
	(*table)[counter_clock] = malloc(sizeof(float) * 4);
	if (!((*table)[counter_clock]))
		return (error_handler(err_alloc_fail));
