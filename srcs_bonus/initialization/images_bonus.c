...
/*   Updated: 2022/01/08 18:45:02 by enena            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_resolution(t_game_master *gm)
{
	...
	if (MIN_WIDTH > ((t_res *)gm->sl->link[resolution].get)->width)
	{
		((t_res *)gm->sl->link[resolution].get)->width = MIN_WIDTH;
		error_handler(wrn_resolution_width_increased);
	}
	if (MIN_HEIGHT > ((t_res *)gm->sl->link[resolution].get)->height)
	{
		((t_res *)gm->sl->link[resolution].get)->height = MIN_HEIGHT;
		error_handler(wrn_resolution_height_increased);
	}
	if (w < ((t_res *)gm->sl->link[resolution].get)->width)
	{
		((t_res *)gm->sl->link[resolution].get)->width = w;
		error_handler(wrn_resolution_width_reduced);
	}
	if (h < ((t_res *)gm->sl->link[resolution].get)->height)
	{
		((t_res *)gm->sl->link[resolution].get)->height = h;
...
{
	const t_res	*res = gm->sl->link[resolution].get;
