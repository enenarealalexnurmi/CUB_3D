...
/*   Updated: 2022/01/08 18:47:30 by enena            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_image	*select_texture(t_game_master *gm, t_sprite *sp)
{
	t_image	*ret;

	ret = NULL;
	if (*(sp->ch) == '2')
		ret = get_anime(gm->sl->link[s_animation].get);
	else if (*(sp->ch) == 'D')
		ret = gm->sl->link[door_close].get;
	else if (*(sp->ch) == 'd')
		ret = gm->sl->link[door_open].get;
	return (ret);
}
...
void	render_frame(t_game_master *gm)
{
	...
	while (++i < gm->map->count_sprite)
		draw_sprite(gm->render->sprites[i], gm->render->rays,
			select_texture(gm, gm->render->sprites[i]), gm->frame);
