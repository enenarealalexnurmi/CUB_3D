...
/*   Updated: 2022/01/08 18:51:47 by enena            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "bmp_utils_bonus.h"
...
t_bool	save_image_as_bmp(t_image *data, const char *name)
{
	...
	bmp_file_name = ft_strjoin(name, BMP_EXTN);
	if (!(bmp_file_name))
		return (error_handler(err_alloc_fail));
	fd = open(bmp_file_name, O_CREAT | O_WRONLY | O_TRUNC, S_IRUSR | S_IWUSR);
	if (fd < 0)
